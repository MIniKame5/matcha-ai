...
import { onAuthStateChangedListener, signInUser, signUpUser, signOutUser } from './services/authService';
import { getChatHistoryList, createNewChat, getChatMessages, updateChat } from './services/firestoreService';
...
const App = () => {
  ...
  const fetchHistory = useCallback(async (user: any) => {
    if (!user) return;
  ...
  const handleSelectChat = async (chatId: string, user = currentUser) => {
    if (!user) return;
    setIsLoading(true);
  ...
  const handleSelectCandidate = async (messageId: number, newIndex: number) => {
    let finalMessagesForSave: MessageType[] = [];
  ...
  const handleSendMessage = async (userInput: string, selectedImage: { base64: string; mimeType: string } | null) => {
    ...
    const modelMessagePlaceholder: MessageType = { id: modelMessageId, role: 'model', content: '' };
    ...
    try {
      ...
      let finalModelMessage: MessageType;
