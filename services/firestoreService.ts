import { collection, doc, getDoc, getDocs, addDoc, updateDoc, serverTimestamp, query, orderBy, limit, DocumentData } from "firebase/firestore";
...
export const getChatHistoryList = async (userId: string): Promise<{ id: string, title: string }[]> => {
...
export const getChatMessages = async (userId: string, chatId: string): Promise<Message[]> => {
...
export const updateChat = async (userId: string, chatId: string, messages: Message[], title?: string) => {
  if (!isFirebaseConfigured || !db || !userId || !chatId) return;
  const chatDocRef = doc(db, 'users', userId, 'chats', chatId);
  try {
