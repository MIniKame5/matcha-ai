...
const Message: React.FC<MessageProps> = ({ message, onSelectCandidate }) => {
        ...
        >
          {message.image && (
            <img 
              ...
              style={{ maxWidth: '200px', maxHeight: '200px' }} // Limit image size in chat bubble
            />
          )}
          {message.content || (isModel && !message.image ? <TypingCursor /> : null)}
        </div>
        {hasCandidates && onSelectCandidate && (
